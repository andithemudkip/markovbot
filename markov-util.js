// ORIGINAL: https://github.com/purplnay/markov-chain
// THIS VERSION IS A SLIGHTLY MODIFIED VERSION OF THE ORIGINAL
// IT ALLOWS THE USER TO SPECIFY A MININUM NUMBER OF WORDS THAT IT SHOULD OUTPUT
// AND ALSO ALLOWS "FROM" TO BE A STRING OF MULTIPLE WORDS
...
class MarkovChain {
    ...
    generate(config = {}) {
      ...
      const maxTries = config.maxTries || this.config.maxTries;
      const minWords = config.minWords || this.config.minWords;
  
      let fromWords = from.split (' ');
      let everyWordInDict = fromWords.every (word => this.contains (word))
  
      if (from !== '' && !everyWordInDict) return ''
      ...
      if (fromWords.length > 1) {
        fromWords.forEach (word => {
          sentence.push (this.dictionary.indexOf (word))
        });
      }
      ...
      while (!done) {
        ...
        if (backward) {
          ...
          done = (tries > maxTries) || ((wordIndex + grams >= chain.length - 1) && (sentence.length > minWords));
        }
      }
